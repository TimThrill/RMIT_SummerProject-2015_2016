...
{
    std::ifstream lexiconfile(std::string("/home/cheetah/Desktop/lexicon").c_str());
    std::ifstream mapFile(std::string("/home/cheetah/Desktop/map").c_str());
    ...
    while(mapFile>>docNo)
    {
            long docOffset;
            mapFile>>docOffset;
            docMap.insert(std::pair<unsigned int, Document>(docNo, Document(docNo, docOffset)));
    }
    ...
    while (lexiconfile>>word) {
            ...
            lexiconMap.insert(std::pair<std::string, Lexicon>(word, Lexicon(word, invlistPos, docFrequency)));
    }
        ...
        Query* queryMessage)
{
    ...
    for(std::vector<std::string>::iterator it_key = queryMessage->getKeyWords().keywords.begin();
            it_key != queryMessage->getKeyWords().keywords.end();
            it_key++)
    {
        if (lexiconMap.find(*it_key) != lexiconMap.end()) {     // The word is in the lexicon map
                ...
                std::ifstream ivsFile(std::string("/home/cheetah/Desktop/ivlist").c_str());
                ...
                for (int j = 0; j < ft; j++) {
                        ...
                        {
                            reviewScore.push_back(std::pair<unsigned int, double>(docId, 0.0)); // This review is not in the review result, add into review results.
                        }
                }
                ivsFile.close();
        }
...
void QueryScore::setRankingResult(int maxResults, QueryReply* queryReplyMessage, std::vector<std::pair<unsigned int, double> >* reviewScore)
{
    std::vector<std::pair<unsigned int, double> >::iterator it = reviewScore->begin();
...
void QueryScore::rankingScore(std::vector<std::pair<unsigned int, double> >* reviewsScore, Query* queryMessage)
{
    float alpha = 0.5;
    double maxDis = 0;
    double maxText = 0;
    ...
    for(std::vector<std::pair<unsigned int, double> >::iterator it = reviewsScore->begin();
            it != reviewsScore->end();
            it++)
    {
        // Read the corresponding json data of this review into memory
        Json::Value root = readReviewJson(docMap[it->first].docOffset);
        EV<<"Processing review: "<<root["hash_value"].asString()<<std::endl;

        Coord businessCoord(root["longitude"].asDouble(), root["latitude"].asDouble());
        double disScore = getDistanceScore(businessCoord, queryMessage->getPeerLocation());
        if(maxDis < disScore)
        {
            maxDis = disScore;
        }
        disScoreset.insert(std::pair<unsigned int, double>(it->first, disScore));
        ...
        textScoreSet.insert(std::pair<unsigned int, double>(it->first, textScore));
        //double reviewScore = textScore * root["rating"].asDouble() / maxText;
        EV<<"Dis socre: "<<disScore<<" text score: "<<textScore<<std::endl<<std::endl;

        ratings.insert(std::pair<unsigned int, double>(it->first, root["rating"].asDouble()));
    }
            ...
            it++)
    {
        ...
        {
            it->second = disScoreset[it->first] / maxDis + textScoreSet[it->first] / maxText + ratings[it->first] / maxRating;
        }
    }

    struct scoreCompare{
        bool operator () (std::pair<unsigned int, double>& a, std::pair<unsigned int, double>& b)
        {
 ...
 * */
double QueryScore::getTextScore(const std::vector<std::pair<unsigned int, double> >::iterator& it_review,
        Query* queryMessage)
{
    int N = queryMessage->getKeyWords().keywords.size();  // number of query word in the keywords collection
    double score = 0;
    for(std::vector<std::string>::iterator it = queryMessage->getKeyWords().keywords.begin();
            it != queryMessage->getKeyWords().keywords.end();
            it++)
    {
...
Json::Value QueryScore::readReviewJson(long line_number)
{
    ...
    std::ifstream jsonFile(std::string("/home/cheetah/Desktop/new"));
